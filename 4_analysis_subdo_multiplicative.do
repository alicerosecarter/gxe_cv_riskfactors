...
Sub-do file for carrying out analyses on the multiplicative scale
*/
...
foreach exp in $grs {
	putexcel set `date'_p`threshold'_trait_multiplicative, sheet(`exp') modify
...
local pheno_out alcohol bmi ldl  
local primary_grs sd_grs_alcohol_`threshold' sd_grs_bmi_`threshold' sd_grs_ldl_`threshold'  

foreach var of varlist `pheno_out' {
	
	gen `var'_plus1 = `var'+1
	gen ln_`var'_plus1 = ln(`var'_plus1)
	egen ln_sd_`var'_plus1 = std(ln_`var'_plus1)
}	

local pheno_out ln_sd_alcohol_plus1 ln_sd_bmi_plus1 ln_sd_ldl_plus1  
...
gen smoking_plus1 = smoking+1
gen ln_smoking_plus1 = ln(smoking_plus1)
egen ln_sd_smoking_plus1 = std(ln_smoking_plus1)

foreach out in ln_sd_smoking_plus1 {
local x = 1
local x = `x'+1

	
	putexcel set csi_results_trait_`threshold'_multiplicative, sheet(all) modify
...
gen sbp_plus1 = sd_sbp+1
gen ln_sbp_plus1 = ln(sbp_plus1)
egen ln_sd_sbp_plus1 = std(ln_sbp_plus1)

foreach out in ln_sd_sbp_plus1 {
local x = 1
local x = `x'+1

	
	putexcel set sbp_results_trait_`threshold'_multiplicative, sheet(all) modify
...
forvalues i = 1/`n' {
	...
	putexcel set `date'_p`threshold'_trait_multiplicative, sheet(`exp') modify	
		...
		logistic `out' `exp' eduyears age sex $PCs `covariate'  if touse_`exp'==1
	...
	putexcel A`x'="`exp_label'" B`x'="`out_label'" C`x'="NONE" D`x'="OR" E`x'=`beta' F`x'=`lci' G`x'=`uci' H`x'=`p_value' J`x'=`n'
...
logistic `out' `exp' age sex  $PCs `covariate' if eduyears==`l' & touse_`exp'==1
	...
	putexcel A`x'="`exp_label'" B`x'="`out_label'" C`x'="Education `l'" D`x'="OR" E`x'=`beta' F`x'=`lci' G`x'=`uci' H`x'=`p_value' J`x'=`n'
...
logit `out' `exp' eduyears `exp'_int age sex $PCs `covariate' if touse_`exp'==1
...
putexcel A`x'="`exp_label'" B`x'="`out_label'" C`x'="Interaction" D`x'="log OR" E`x'=`beta_int' F`x'=`lci_int' G`x'=`uci_int' H`x'=`p_value' 
...
foreach touse in  touse_sd_grs_smoking_`threshold' touse_sd_grs_sbp_`threshold'  {
		...
		putexcel set `date'_p`threshold'_trait_multiplicative, sheet(`sheet') modify
...
import excel "$resDir/data/results/genetic/202108/csi_results_trait_`threshold'_multiplicative.xlsx", sheet("all") firstrow clear
...
gen eduyears_interact = 1 if eduyears_csi=="EA interact"
	...
	putexcel set `date'_p`threshold'_trait_multiplicative, sheet(sd_grs_smoking_`threshold') modify
	putexcel A1="Exposure" B1="Outcome" C1="Interaction" D1="Scale" ///
...
import excel "$resDir/data/results/genetic/202108/sbp_results_trait_`threshold'_multiplicative.xlsx", sheet("all") firstrow clear
destring sample, replace
lab var sample "Sample"
lab def sample 1 "Sample 1" 2 "Sample 2"
lab val sample sample
...
gen eduyears_interact = 1 if eduyears_sbp=="EA interact"
	...
	metan or lci uci if  eduyears_level==. & eduyears_interact==., lcols(sample ) effect(OR) null(1) xlabel(-0.0001, -0.001, -0.01, 0.0) astext(50) dp(6) nograph
	putexcel set `date'_p`threshold'_trait_multiplicative, sheet(sd_grs_sbp_`threshold') modify
	putexcel A1="Exposure" B1="Outcome" C1="Interaction" D1="Scale" ///
		E1="Beta/OR" F1="LCI" G1="UCI" H1="P Value" I1 = "P value for interaction (with EA)" ///
