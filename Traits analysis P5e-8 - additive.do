...
foreach exp in $primary_grs {
	putexcel set 202008_p5e8_trait_additive, sheet(`exp') modify
...
********************************************************************************
*Per unit increase in GRS

** Not including split sample variables, SBP and CSI - will run separately in script later

local pheno_out sd_alcohol af sd_bmi cad ckd diabetes_t2 sd_ldl mdd migraine ra sle stroke sd_total_chol 
local primary_grs sd_grs_alcohol_5e8 sd_grs_af_5e8 sd_grs_bmi_5e8  sd_grs_cad_5e8 sd_grs_ckd_5e8 sd_grs_diabetes_t2_5e8 sd_grs_ldl_5e8 sd_grs_mdd_5e8 sd_grs_migraine_5e8 sd_grs_ra_5e8 sd_grs_sle_5e8 sd_grs_stroke_5e8 sd_grs_total_chol_5e8 
...
forvalues i = 1/`n' {
	...
	putexcel A`x'="`exp_label'" B`x'="`out_label'" C`x'="NONE" D`x'="Difference" E`x'=`beta' F`x'=`lci' G`x'=`uci' H`x'=`p_value' J`x'=`n'
...
local x = `x'+1
	...
	putexcel set csi_results_trait_5e8_additive, sheet(all) modify
...
local x = `x'+1
	...
	putexcel set sbp_results_trait_5e8_additive, sheet(all) modify
...
foreach touse in  touse_sd_grs_smoking_5e8 touse_sd_grs_sbp_5e8  {
		...
		putexcel set 202008_p5e8_trait_additive, sheet(`sheet') modify
...
import excel "csi_results_trait_5e8_additive.xlsx", sheet("all") firstrow clear
...
gen eduyears_interact = 1 if eduyears_csi=="EA interact"
	...
	putexcel set 202008_p5e8_trait_additive, sheet(sd_grs_smoking_5e8) modify
	putexcel A1="Exposure" B1="Outcome" C1="Interaction" D1="Scale" ///
...
import excel "sbp_results_trait_5e8_additive.xlsx", sheet("all") firstrow clear
destring sample, replace
lab var sample "Sample"
lab def sample 1 "Sample 1" 2 "Sample 2"
lab val sample sample
...
gen eduyears_interact = 1 if eduyears_sbp=="EA interact"
	...
	metan or lci uci if  eduyears_level==. & eduyears_interact==., lcols(sample ) effect(OR) null(1) xlabel(-0.0001, -0.001, -0.01, 0.0) astext(50) dp(6) nograph
	putexcel set 202008_p5e8_trait_additive, sheet(sd_grs_sbp_5e8) modify
	putexcel A1="Exposure" B1="Outcome" C1="Interaction" D1="Scale" ///
		E1="Beta/OR" F1="LCI" G1="UCI" H1="P Value" I1 = "P value for interaction (with EA)" ///
	...
	local uci = (r(ci_upp))
	local p_val = r(p_z)	
	putexcel A`x'="SBP combined P=5x10-8 (SD)" B`x'="SBP (SD)" C`x'="NONE" D`x'="Difference" E`x'=`beta' F`x'=`lci' G`x'=`uci' H`x'=`p_val'
...
foreach l of local levels {
	...
	local uci = (r(ci_upp))
	local p_val = r(p_z)	
	putexcel A`x'="SBP combined P=5x10-8 (SD)" B`x'="SBP (SD)" C`x'="Education `l'" D`x'="Difference" E`x'=`beta' F`x'=`lci' G`x'=`uci' H`x'=`p_val'
