...
import excel "$resDir/data/results/genetic/202108/`date'_p`threshold'_trait_additive", sheet("`var'") firstrow clear
tempfile `var'
save `var'_additive, replace

}

use sd_grs_alcohol_`threshold'_additive, clear
foreach var in  sd_grs_af_`threshold'_additive sd_grs_bmi_`threshold'_additive  sd_grs_cad_`threshold'_additive sd_grs_diabetes_t2_`threshold'_additive sd_grs_ldl_`threshold'_additive sd_grs_stroke_`threshold'_additive  sd_grs_smoking_`threshold'_additive sd_grs_sbp_`threshold'_additive {
...
twoway (rspike lci uci row if eduyears==1, lcolor(erose) vertical) ///
		...
		legend(size(vsmall)) ///
		ylabel(0.0 "0.0" 0 "0.0" 0.05 "0.05"  0.10 "0.10" 0.15 "0.15" 0.20 "0.20" 0.25 "0.25" 0.30 "0.30", angle(0) labsize(vsmall)) ///
		xlabel(4 "Alcohol"  12 "BMI" 20  "LDL-C" 28 "Smoking" 36  "SBP", angle(0) noticks labsize(vsmall)) ///
		xtitle("Continuous Trait",color(black) size(small)) ytitle("Mean difference in phenotypic trait",color(black) size(small)) ///
		aspect(.5) graphregion(color(white))

graph export PGS_`threshold'_continuous_additive_`date'.eps, replace
...
twoway (rspike lci uci row if eduyears==1, lcolor(erose) vertical) ///
		...
		legend(row(1) order(2 "No interaction" 4 "7 years" 6 "10 years" 8 "13 years" 10 "15 years" 12 "19 years" 14 "20 years")) ///
		legend(size(vsmall)) ///
		ylabel(-0.005 "-0.005" 0.000 "0.000" 0.005 "0.005" 0.010 "0.010" 0.015 "0.015" 0.020 "0.020" 0.025 "0.025", angle(0) labsize(vsmall)) ///
		xlabel(4.5 "AF" 12 "CHD" 20 "T2D" 27.5 "Stroke", angle(0) noticks labsize(vsmall)) ///
		xtitle("Binary Trait",color(black) size(small)) ytitle("Risk difference in outcome",color(black) size(small)) ///
		aspect(.5) graphregion(color(white))

graph export PGS_`threshold'_binary_additive_`date'.eps, replace
